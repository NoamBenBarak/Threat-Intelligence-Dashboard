...
const fetchFromAPIs = async (ip) => {
  ...
  const [abuseIpDbResponse, ipQualityScoreResponse] = await Promise.all([
    abusePromise,
    ipqsPromise,
  ]);
  ...
  const countryName =
    getName(ipQualityScoreData?.country_code || abuseIpDbData?.countryCode) ||
    DEFAULT_VALUES.COUNTRY;

  const threatScoreValue =
    calculateDerivedThreatScore(ipQualityScoreData, abuseIpDbData) ??
    DEFAULT_VALUES.THREAT_SCORE;
  ...
  const result = {
    ...
    country: countryName,
    abuseScore:
      abuseIpDbData?.abuseConfidenceScore || DEFAULT_VALUES.ABUSE_SCORE,
    recentReports: abuseIpDbData?.totalReports || DEFAULT_VALUES.RECENT_REPORTS,
    vpnOrProxyDetected:
      ipQualityScoreData?.vpn ||
      ipQualityScoreData?.proxy ||
      DEFAULT_VALUES.VPN_DETECTED,
